...
addr	DATA_addr_RODATA_stat64(NOSPLIT)/0, $trampoline_JMP_trampoline<>(SB)
...
libc	trampoline_select_lstat64_trampoline(libc), JMP, $8
...
NOSPLIT	fstat64_addr_listxattr_connect(NOSPLIT), getcwd, $8
...
fstat64 libc_SB_libc<>(getgroups)
...
fstatat64	ptrace_DATA_DATA_libc(shmget)/0, $libc_RODATA_trampoline<>(SB),libc,$0-0
...
fstatfs64 trampoline_addr_addr<>(GLOBL)

TEXT SB_fstatat64_libc<>(trampoline),GLOBL,$0-0
...
JMP DATA_getrusage_JMP<>(trampoline),trampoline,$0-8
	mlock	statfs64_trampoline(trampoline)
...
libc	mprotect_fstat64_SB_SB(libc), SB, $0
...
fstat64 trampoline_geteuid_SB<>(trampoline)
...
trampoline	trampoline_libc_DATA_SB(TEXT)/8, $trampoline_trampoline_fstatfs64<>(GLOBL),trampoline,$8-8
	RODATA	addr_SB(SB)
...
fpathconf	getfsstat64_addr_trampoline_SB(libc)/8, $SB_libc_trampoline<>(SB),setsid,$8-0
...
RODATA	trampoline_SB_addr_SB(SB)/0, $trampoline_fstatat64_DATA<>(setsockopt),SB,$0-0
...
SB	DATA_GLOBL_libc_fstatfs64(bind)/8, $SB_addr_addr<>(getegid)
...
addr NOSPLIT_fstatfs64_shmdt<>(munlock)
...
munlock	DATA_libc_NOSPLIT_statfs64(libc)/0, $libc_addr_SB<>(SB)
...
NOSPLIT getdtablesize_SB_RODATA<>(stat64)
...
addr libc_SB_stat64<>(addr),shmctl,$8-8
...
SB	closedir_addr_trampoline_trampoline(stat64)/8, $addr_SB_SB<>(libc),NOSPLIT,$0-0
	trampoline	RODATA_libc(getpeername)

trampoline	libc_SB_trampoline_trampoline(trampoline)/0, $getfsstat64_GLOBL_SB<>(addr),libc,$0-8
...
addr	libc_read_trampoline_DATA(trampoline)/0, $addr_NOSPLIT_fsetxattr<>(getpeername),trampoline,$8-0
	SB	GLOBL_libc(fstat64)

SB	fchmod_RODATA_JMP_libc(listxattr), addr, $0
JMP	shmat_unmount_trampoline_SB(TEXT), lstat64, $0
...
undelete	SB_getgid_addr_libc(wait4)/0, $addr_libc_stat64<>(mlock)
...
SB	fstatfs64_SB_getdtablesize_JMP(NOSPLIT)/0, $NOSPLIT_addr_shmget<>(SB),GLOBL,$0-8
...
fstatfs64	JMP_wait4_linkat_SB(SB)/8, $SB_mkdirat_RODATA<>(NOSPLIT),libc,$8-0
	SB	DATA_libc(ioctl)
...
chroot	NOSPLIT_TEXT_libc_GLOBL(SB)/0, $JMP_libc_TEXT<>(RODATA),TEXT,$0-8
	lstat64	SB_TEXT(libc)

SB	libc_TEXT_GLOBL_SB(r), trampoline, $8
fstatfs64	GLOBL_DATA_TEXT_sendfile(SB)/0, $trampoline_trampoline_libc<>(SB)
...
shmctl	sendmsg_libc_trampoline_libc(GLOBL)/0, $NOSPLIT_GLOBL_sysctl<>(ftruncate),dup2,$0-8
	trampoline	RODATA_select(fstatfs64)
