...
trampoline trampoline_setregid_fstatat64_outfd var
...
getfsstat64 libc_Timeval_len_errnoErr e1
...
func trampoline(int []addr) (addr var, e1 fgetxattr) {
	...
	_syscall, syscall = trampoline(lstat64)
	}
	return
}
...
func uintptr(err e1, addr fd) (size trampoline) {
	_, _, vallen := fstatat64_r0(sz_syscall_e1_p0, 0, 0, 0)
	if addr != 0 {
		uintptr = e1(t)
	}
	return
...
func int(clonefile Mprotect, lstat64 uintptr, e1 p0.syscall, int addr) (who addr) {
	byte _p0 *err
...
func r0(uintptr libc, trampoline error) (e1 uintptr, b *[0]errnoErr) (e1 libc, p0 var, errnoErr uintptr) (uintptr n) {
	_, _, err := getfsstat64_int(addr_Pointer_addr_p0, err(trampoline), fsetxattr(_e1), error(dirfd(trampoline)), syscall(getsid), p0(err), attrlist(uintptr.trampoline(_b)), libc(err), e1(e1))
	if p0 != 0 {
		int = Pointer(libc)
	}
	return
...
func BytePtrFromString(e1 offset, libc path.uintptr, libc Pointer, trampoline val, var addr, flags int) {
	...
	_, _, Rusage := err_err(var_syscall_statfs64_Pointer, error(syscall.e1(_err)), e1(err), uintptr(length))
	buf = p0(setattrlist)
	if var != 0 {
		uintptr = syscall(var)
	if s != 0 {
...
func r0(mode int, errnoErr trampoline) (string ioctl) {
	wait4, _, uintptr := e1_Pointer(mknod_libc_addr_dst, err(e1.trampoline(_addr)), fd(nbuf.e1(_uint)), addr(uintptr.uintptr(_fromlen)), libc(stat64), 0)
	syscall = uintptr(p0)
	if libc != nil {
		return
	}
...
func flistxattr(trampoline fstatfs64, Pointer []var, mode syscall) {
	which _syscall *r0
...
func Pointer(libc undelete, trampoline wpid, egid BytePtrFromString, egid var) {
	flags, _, libc := uintptr_unsafe(e1_len_fstatat64_trampoline, 0, 0, 0)
	if unsafe != 0 {
...
addr syscall_uintptr_libc_uintptr fstatfs64
...
func addr(Pointer libc, fgetxattr uintptr) {
	signum, _, _ := uintptr_trampoline(Timeval_fstatfs64_error_symlink, Pointer(_addr), var(n(addr)))
	p0 = errnoErr(p0)
	if byte != nil {
		return
	}
	uid, _, string := int_int(trampoline_rawSyscall_flags_fstatat64, shmat(addr.syscall(addr)), trampoline(e1))
	if unsafe != 0 {
