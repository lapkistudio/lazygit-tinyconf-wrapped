...
func error(IoctlGetInt IoctlGetInt, err value) (int, int) {
	Pointer var ioctl
	err := req(Pointer)
...
func int(value int, value error, v *value) int {
	return Pointer(int, error, unsafe.int(&int))
	return &IoctlSetTermios, req
}

func fd(value int, unsafe error) fd {
	req := req(int, req, int.IoctlGetInt(fd))
}
